...
Properties {
	...
	//sampler2D test;
	float4 _Cam1_TexelSize;
	float4 _Cam2_TexelSize;
	float4 _Cam3_TexelSize;
	float4 _Cam4_TexelSize;
	...
	float4 frag(v2f i) : SV_Target {
		...
		float4 leftColor = tex2D(_Cam1, float2((i.pos.x%(1/_Cam1_TexelSize.x)) * _Cam1_TexelSize.x, (-(i.pos.y * _Cam1_TexelSize.x))+1));
	...
	//float2 posP = float2(xp, yp);
		...
		if(i.pos.x > (1/_Cam1_TexelSize.x) && i.pos.x < (1/_Cam1_TexelSize.x)*2){
			...
			for(int j = 0 ; j < 256; j++){
				...
				realCameraColors = tex2D(_Cam1, float2(j/(1/_Cam1_TexelSize.x), (-(i.pos.y * (_Cam1_TexelSize.x)))+1));
				...
				xe = ( (j-((1/_Cam1_TexelSize.x) / 2)) * -ze)/-_ImagePlaneLength;
				...
				//Convert back from eye space to projection space
				 ...
				 xp = xp + ((1/_Cam1_TexelSize.x)/2);
				// yp = (_ImagePlaneLength * posE.y) / ze;

				//Combine to vector2
				 posP = float2(xp, yp);

				if(abs((xp) - (i.pos.x - (1/_Cam1_TexelSize.x))) < 2){
			...
			float test = (i.pos.x - (1/_Cam1_TexelSize.x)) / (1/_Cam1_TexelSize.x);
			//return float4(test,test,test,1);
			return float4(1,0,0,0);

		}

		if(i.pos.y > (1/_Cam1_TexelSize.y)){
